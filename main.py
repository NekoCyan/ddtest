...
def countdown(t):
    ...
    while True:
        if (until - datetime.datetime.now()).total_seconds() > 0:
            stdout.flush()
            stdout.write("\r " + Fore.MAGENTA + "[*]" + Fore.WHITE + " Đang Tấn công = " + str(
                (until - datetime.datetime.now()).total_seconds()) + " thời gian ")
        else:
            stdout.flush()
            stdout.write(
                "\r " + Fore.MAGENTA + "[*]" + Fore.WHITE + "  Done !                                   \n")
            return
